...
class PageGtk(PageBase):
    ...
    def plugin_on_next_clicked(self):
        ...
        if not partserv:
            syslog.syslog("JACKJUMP: How are we doing on memory? A valid dictionary copy of layout, subset of PartedServer disk partitions, must exist in order to proceed (which should have been generated by now).")
        ...
        if install_device:
            ...
            # partserv     1: size 3: fs 0: dev
            for partition in partserv[install_disk]:
        ...
        ####install_links.append([partition[0], ])
        else:
        ...
        if copy_device:
            ...
            # PartedServer 2: size 4: fs 5: dev
            # partserv     1: size 3: fs 0: dev
            for partition in partserv[copy_disk]:
                if partition[3] == 'BitLocker':  # fs
            ...
            if not copy_mounted:
                ...
                self.extra_options['jackjump'] = copy_drive_not_mounted
                return
            ...
            syslog.syslog("JACKJUMP: A valid copy drive with a supported filesystem must be selected in order to proceed.")
            #title = ('Please select a valid copy drive.')
            no_copy_drive = 'ubiquity/text/no_copy_drive'
            self.extra_options['jackjump'] = no_copy_drive
            return
        ...
        if not install_has_windows and not copy_has_windows:
            ...
            no_windows = 'ubiquity/text/no_windows'
        ...
        if copy_device and install_parts and copy_fs_dev and install_has_windows:
            ...
            if install_used and copy_free > 0:
                ...
                if source_size > (dest_free * 1.1):
                    ...
                    self.extra_options['jackjump'] = copy_drive_small
                    return
            ...
            if not os.path.exists(mount_point):
                misc.execute_root('mkdir', '-p', mount_point)
            #title = ('Copying user files to copy drive')
            user_to_copy_drive = 'ubiquity/text/user_to_copy_drive'
            msg = self.controller.get_string(user_to_copy_drive) 
            from gi.repository import Gtk
            dialog = Gtk.MessageDialog(
                self.current_page.get_toplevel(), Gtk.DialogFlags.MODAL,
            ...
            for part in install_parts:
                if not misc.execute_root('mount', '-t', 'ntfs', '-o', 'ro', part[0], mount_point):
                    ...
                    self.extra_options['jackjump'] = install_drive_not_mounted
                    misc.execute_root('rmdir', '--ignore-fail-on-non-empty', mount_point)
                    return
                    ...
                    for main_path in part[1]:
                        ...
                        if os.path.exists(source_path):
                            ...
                            if not misc.copy_to_drive_root(source_path, copy_fs_dev, dest_path, db=None, was_compressed=use_compression):
                                ...
                                self.extra_options['jackjump'] = no_user_files
                                misc.execute_root('umount', mount_point)
                                misc.execute_root('rmdir', '--ignore-fail-on-non-empty', mount_point)
                ...
                misc.execute_root('umount', mount_point)
            # Subpart 3.2: Copy user link files to jackjump/users
            for part in install_links:
                if not misc.execute_root('mount', '-t', 'ntfs', '-o', 'ro', part[0], mount_point):
                    syslog.syslog(f"JACKJUMP: The partition {part[0]} on install drive with Windows has an issue or something is making it unable to mount.")
                    #title = ('Windows install drive not mounted.')
                    install_drive_not_mounted = 'ubiquity/text/install_drive_not_mounted'
                ...
                if os.path.exists(mount_point):
                    ...
                    syslog.syslog(f"JACKJUMP: Mount point {mount_point} doesn't exist. Installation cannot proceed.")
                    #title = ('No Users directory on install drive.')
                    no_users_dir = 'ubiquity/text/no_users_dir'
                    self.extra_options['jackjump'] = no_users_dir
                    misc.execute_root('umount', mount_point)
...
class Page(plugin.Plugin):
    ...
    def run(self, priority, question):
        ...
        if question.endswith('automatically_partition'):
            ...
            with misc.raised_privileges():
                ...
                # steve@jackjump.com/grok3 added partserv for copy drive
                if 'partserv' not in self.extra_options:
                    import copy
                    partserv = copy.deepcopy(layout)
                    self.extra_options['partserv'] = partserv
    ...
    def ok_handler(self):
        ...
        if self.current_question.endswith('automatically_partition'):
            ...
            # steve@jackjump.com/grok3 added variable pass to debconf 
            if 'jackjump' in self.extra_options:
                jackjump = self.extra_options.get('jackjump')
                response = self.frontend.question_dialog(
                    self.description(jackjump),
                    self.extended_description(jackjump),
                    ('ubiquity/text/stop',))
                if response == 'ubiquity/text/stop':
                    import sys
                    sys.exit(1)
                else:
                    del self.extra_options['jackjump']
            if 'copy_fs_dev' in self.extra_options:
                copy_fs_dev = self.extra_options.get('copy_fs_dev')
                import json
                self.db.set('ubiquity/copy_fs_dev', json.dumps(copy_fs_dev))
            if 'copy_parts' in self.extra_options:
            ...
            if 'copy_has_windows' in self.extra_options:
                copy_has_windows = self.extra_options.get('copy_has_windows', False)
                self.db.set('ubiquity/copy_has_windows', 'true' if copy_has_windows else 'false')
            if 'copy_compressed' in self.extra_options:
                copy_compressed = self.extra_options.get('copy_compressed', False)
                self.db.set('ubiquity/copy_compressed', 'true' if copy_compressed else 'false')
