...
class PageGtk(PageBase):
    ...
    def plugin_on_next_clicked(self):
        ...
        if not partserv:
            ...
            msg = self.controller.get_string(no_install_drive)
            from gi.repository import Gtk
            dialog = Gtk.MessageDialog(
                self.current_page.get_toplevel(), Gtk.DialogFlags.MODAL,
                Gtk.MessageType.ERROR, Gtk.ButtonsType.CLOSE, None)
        ...
        if copy_device:
            ...
            syslog.syslog("JACKJUMP: A valid copy drive with a supported filesystem must be selected in order to proceed. Otherwise, you're just installing - not migrating anything. Which will take extra overhead time for no reason and possibly not work.")
            #title = ('Please select a valid copy drive.')
            no_copy_drive = 'ubiquity/text/no_copy_drive'
            msg = self.controller.get_string(no_copy_drive)
            from gi.repository import Gtk
        ...
        if copy_device and install_part and install_parts and copy_fs_dev and install_has_windows:
            ...
            for part in install_parts:
                ...
                if os.path.exists(source_dir):
                    if not misc.copy_to_drive_root(source_dir, copy_fs_dev, 'jackjump/users', db=None, was_compressed=use_compression):
                        misc.execute_root('umount', mount_point)
                        syslog.syslog(f"JACKJUMP: Failed to copy Windows user files from install partition {part} to copy drive {copy_fs_dev[1]}. Installation cannot proceed.")
                ...
                for src in windows_data_sources:
                    for path in glob.glob(src):
                        ...
                        if (len(dirs) >= 4 and (dirs[3] not in windows_users_extra_dirs or not dirs[3].startswith('Defaultuser'))) and os.path.exists(path):
                            ...
                            if not misc.copy_to_drive_root(path, copy_fs_dev, os.path.join('jackjump/windows_data', dirs_bottom), db=None, was_compressed=use_compression):
